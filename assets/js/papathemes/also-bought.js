...
export default class AlsoBought {
    ...
    updateAddSelectedToCartButton() {
        ...
        if ($checked.length > 0) {
            const strAll = $checked.length <= this.allNumberTexts.length ? this.allNumberTexts[$checked.length - 1] : $checked.length;
            const strNum = $checked.length <= this.numberTexts.length ? this.numberTexts[$checked.length - 1] : $checked.length;
            const str = $checked.length === $all.length ? strAll : strNum;
